...
export default function WaitRoom() {
  ...
  const buttonEnabled = useMemo(() => {
    if (users.length > 0) {
      return (users.filter((user) => user.isHost)[0].publicKey === wallet.publicKey?.toBase58()) && users.length >= 2 
    } else {
      return false;
    }
  }, [users])
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-between p-24">
      <NavBar />
      <div className="flex flex-col h-[450px] max-w-[830px] w-full items-center justify-center z-10">
        <Room users={users}/> 
      <Button className="w-[500px] h-[100px] rounded-xl italic ring-8 ring-orange-600 ring-offset-3 ring-offset-black hover:bg-[#f7d726] text-[64px] flex items-center justify-center mt-10 transition ease-in-out hover:-translate-y-1 hover:scale-110" disabled={!buttonEnabled} onClick={onStart}>
      ...
      <div className="mt-10 ">
      {leaderBoard && leaderBoard.map((l, i) => <h3 key={l[0]} className="font-sans text-lg text-shadow-custom text-[#8DFCBC]">{`#${i+1} ${l[0].slice(0, 6)}...${l[0].slice(l[0].length-3, l[0].length-1)}: ${l[1]}`}</h3>)}
      </div>
      </div>
